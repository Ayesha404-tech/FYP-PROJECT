...
export const NotificationCenter: React.FC<NotificationCenterProps> = ({ isOpen, onClose }) => {
  ...
  const notifications: Notification[] = useQuery(api.notifications.getNotifications, user ? { userId: user.id as any } : "skip") || []; // eslint-disable-line @typescript-eslint/no-explicit-any
  ...
  const markAsRead = async (id: string) => {
    try {
      await markAsReadMutation({ notificationId: id as any }); // eslint-disable-line @typescript-eslint/no-explicit-any
    } catch (error) {
  ...
  const markAllAsRead = async () => {
    if (!user) return;
    try {
      await markAllAsReadMutation({ userId: user.id as any }); // eslint-disable-line @typescript-eslint/no-explicit-any
    } catch (error) {
